...
description: >
  ...
  and optional Canva doc. Features: SPIKE/WATCH/LOCAL/ELEVATED theme tagging, churn analysis
...
>
> 📄 **Canva doc** — narrative write-up with theme breakdowns, verbatim quotes, and operating plan connections. Generated by copying your branded **User Voice Insights Newsletter** template each week — layout and styling are consistent every run. Heads up: docs are currently **view-only** (recipients can't comment or edit).
>
...
>
> 📄 **Running Canva doc archive** — I'll generate a doc each week and build up a running archive, newest first. Worth knowing: each week's doc is freshly generated so styling may vary — it's best used as a working reference rather than a polished share. Want me to set that up?
>
...
**Canva doc archive — generate-design approach.** If the manager says yes to the archive doc: on first run, generate a doc via `generate-design` and store the resulting design ID as `canva_doc_archive`. On all subsequent runs, generate a fresh doc and prepend it to the stored archive via `merge-designs`. Each week's doc is freshly generated — visual consistency is not guaranteed. Do not overpromise on formatting quality.
...
If the manager opted in to the archive doc during onboarding:

**Every run:** Generate the week's content as a fresh doc using `generate-design` with `design_type: "doc"`. Visual styling will vary run to run — this is a known limitation of the current integration.

**First run (no archive ID stored yet):**
1. Generate the week's doc.
2. Store the resulting design ID as `canva_doc_archive` in the scheduled task prompt.
3. Present the doc URL to the manager: "Your first digest doc is ready — bookmark this, I'll add to it each week."

**Subsequent runs (archive ID stored):**
1. Generate the week's doc as above.
2. Call `merge-designs` to prepend the new doc to the stored archive — newest first.
3. Present the archive URL (unchanged — same link every week).

**Note on quality:** Because each week's doc is freshly generated rather than filled from a fixed template, layout and styling will not be perfectly consistent. This is a current integration limitation. Do not present the doc as a polished branded deliverable — frame it as a working reference document.
...
### Tier 2: Canva Doc (include if delivery includes "doc")

Create a Canva doc by copying the **User Voice Insights Newsletter** template and filling its placeholders.

**Template design ID:** `DAHRkgUutkg`
**Placeholder fields:** `{{Date}}` and `{{Summary}}` (both in the page body, element ID `PBHwrtX0FYYbpb4Z`)

**Steps:**

1. Call `mcp__b7e20176...copy-design` with `design_id: "DAHRkgUutkg"` — creates a fresh copy of the branded template.
2. Note the new copy's `design.id` from the response.
3. Call `mcp__b7e20176...read-design` on the copy with `open_transaction: true` — note the `transaction_id`.
4. Call `mcp__b7e20176...edit-design` with the transaction, `page_index: 1`, `is_responsive: true`, and two operations:
   - `{ type: "find_and_replace_text", element_id: "PBHwrtX0FYYbpb4Z", find_text: "{{Date}}", replace_text: "Week of [WEEK_START] – [WEEK_END]" }`
   - `{ type: "find_and_replace_text", element_id: "PBHwrtX0FYYbpb4Z", find_text: "{{Summary}}", replace_text: "[FULL_NARRATIVE]" }`

   Note: run these as **two separate edit-design calls** — one operation per call on responsive pages.
5. Call `edit-design` with `finalize: "commit"` to save.
6. Present `design.urls.edit_url` (or `view_url`) to the user.

**`{{Summary}}` content spec — compose as flowing prose:**
...
**Why markdown:** Unlike the Canva doc (which is generated fresh each week with variable styling), markdown is structurally consistent every run. It pastes cleanly into a Canva doc, opens in Cursor, Obsidian, Notion, or any markdown editor, and works well for people who maintain a second-brain or want to version-control their digests.
